...
exports.scrapeTesco = scrapeTesco;
const normalise_js_1 = require("../utils/normalise.js");
const removeImages_js_1 = require("../utils/removeImages.js");
...
async function scrapeTesco(queries, maxResultsPerQuery) {
    const results = [];
...
function extractTescoProductsFromHtml(html, query, maxResults) {
    ...
    return productRecords.slice(0, limit).map((product, index) => {
        ...
        const href = product.tpnc
            ...
            : null;
        const promotionText = product.promotions?.[0]?.promotionText ||
        ...
        return {
            ...
            price: (0, normalise_js_1.toNumber)(priceText),
            ...
            in_stock: typeof product.isForSale === 'boolean'
                ? product.isForSale
                : product.status
                    ? /available/i.test(product.status)
                    : null,
            product_url: (0, normalise_js_1.absoluteTescoUrl)(href),
            ...
            rating: null,
            review_count: null,
            raw_data: (0, removeImages_js_1.removeImageFields)({
                query,
                position: index + 1,
                product,
            }),
...
function extractProductRecords(html) {
    const records = [];
    const seen = new Set();
    ...
    while ((match = regex.exec(html)) !== null) {
        ...
        try {
            ...
            if (!product.title || seen.has(productId))
                continue;
            seen.add(productId);
            records.push({
                ...product,
